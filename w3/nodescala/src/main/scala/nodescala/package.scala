...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs match {
        case Nil => Future(Nil)
        case fsh :: fst => fsh.flatMap(head => all(fst).flatMap(tail => Future(head :: tail)))
      }
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach(ft => ft onComplete {
        case Success(t) => p.complete(Try(t))
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      f.value match {
        ...
        case Some(x) => x match {
          case Success(t) => t
          case failure @ Failure(er) => throw er
        }
      }
    }
